...
def main():
    ...
    print(f"Found headers at index: {headerRowIndex}")
    
    # Print the three rows starting from headerRowIndex
    for i in range(3):
        idx = headerRowIndex + i
        if idx < len(rows):
    ...
    # Print a few Preksha rows
    for r_idx, ut, row in preksha_rows[:15]:
        print(f"Row {r_idx+1} ({ut}): {row}")
