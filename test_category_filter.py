...
def main():
    ...
    colIdx = {
        ...
        'finalStatus': 16, 'businessDevelopment': 17, 'dependency': 18,
    ...
    for idx, s in enumerate(combinedHeaders):
        ...
        elif ('requirement' in s or 'requiement' in s or 'requiemernt' in s) and 'change' not in s and 'new' not in s and 'approval' not in s and 'cr' not in s:
            colIdx['reqGathering'] = idx
        ...
        elif 'dynamic' in s and 'status' not in s and 'review' not in s and 'detail' not in s and 'approval' not in s:
            colIdx['dynamicDev'] = idx
        ...
        elif 'business' in s or 'bd' in s or 'biz' in s: colIdx['businessDevelopment'] = idx
    ...
    for k, v in colIdx.items():
        print(f"  {k}: {idx_str(v, combinedHeaders)}")
    ...
    for r in range(dataStartRowIndex, len(rows)):
        ...
        # Capture requirement gathering value
        reqGatheringVal = str(row[colIdx['reqGathering']]).strip() if colIdx['reqGathering'] < len(row) else ''
        if reqGatheringVal and reqGatheringVal != '-' and mod['requirementGathering'] == '-':
            mod['requirementGathering'] = reqGatheringVal
    ...
    print("\nParsed Module Stats & Requirement Gathering:")
    for mid, mod in parsedModules.items():
        ...
        print(f"  {mid:12}: Pages = {total:2}, Done = {done:2}, Req Gathering = '{mod['requirementGathering']}'")

def idx_str(idx, headers):
    if idx < len(headers):
        return f"{idx} -> '{headers[idx]}'"
    return f"{idx} -> OUT OF BOUNDS"
