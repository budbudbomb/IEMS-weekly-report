...
def main():
    ...
    max_len = max(len(row1), len(row2), len(row3))
    
    # Pad rows
    row1 += [''] * (max_len - len(row1))
    row2 += [''] * (max_len - len(row2))
    row3 += [''] * (max_len - len(row3))
    
    print("INDEX | ROW 1 | ROW 2 | ROW 3")
    print("-" * 60)
    for i in range(max_len):
        r1 = row1[i].strip().replace('\n', ' ')
        r2 = row2[i].strip().replace('\n', ' ')
        r3 = row3[i].strip().replace('\n', ' ')
        print(f"{i:5} | {r1:25} | {r2:25} | {r3:25}")
