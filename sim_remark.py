...
# Simulate the parser's remark aggregation for Preksha
current_module = ''
current_usertype = ''
module_remarks = {}  # module_id -> first remark found

for i, row in enumerate(rows[3:], start=4):  # skip 3 header rows
    ...
    if remark_val and str(remark_val).strip():
        mid = current_module.lower().replace(' ', '_')
        if mid not in module_remarks:
            module_remarks[mid] = str(remark_val).strip()
            print(f"Row {i}: SET remark for module='{current_module}' ut='{current_usertype}': '{str(remark_val)[:60]}'")
        else:
            print(f"Row {i}: SKIP (already set) for module='{current_module}' ut='{current_usertype}': '{str(remark_val)[:60]}'")

print("\n\n=== Final module remarks ===")
for k,v in module_remarks.items():
    print(f"  {k}: '{v[:80]}'")
