...
def main():
    ...
    with zipfile.ZipFile(xlsx_path, 'r') as z:
        ...
        print("\n--- Rows 50 to 110 ---")
        for i in range(50, min(110, len(rows))):
            row_data = rows[i]
            # Print only non-empty elements with index to see columns clearly
            print(f"Row {i+1}: " + " | ".join(f"{c_idx}:{val}" for c_idx, val in enumerate(row_data) if val != ''))
