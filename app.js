...
function nextSlide() {
    if (!presentationMode) return;
    if (currentSlideIndex < MODULE_ORDER.length + 2) {
        currentSlideIndex++;
        renderSlide();
    }
}
...
function renderSlide() {
    ...
    document.getElementById('pres-next-btn').disabled = (currentSlideIndex === MODULE_ORDER.length + 2);

    let dotsHtml = '';
    for (let i = 0; i <= MODULE_ORDER.length + 2; i++) {
        let title = '';
        let color = '';
        if (i === 0) {
            title = 'Overview';
...
function renderJuneCommitments(containerId) {
    ...
    JUNE_COMMITMENTS.forEach(commit => {
        ...
        html += `
            <div class="new-pres-card" style="border-top: 4px solid ${mod.color}; padding: 1.25rem; height: 100%; display: flex; flex-direction: column; justify-content: flex-start; gap: 1rem; box-shadow: var(--shadow-md); min-height: 0; overflow: hidden;">
                <div class="pres-card-header" style="border-bottom: 1px solid var(--border-subtle); padding-bottom: 0.75rem; flex-shrink: 0;">
                    <div style="display: flex; align-items: center; gap: 0.75rem;">
                        <div class="detail-module-icon" style="background: ${mod.color}15; color: ${mod.color}; border: 1px solid ${mod.color}25; width: 36px; height: 36px; font-size: 1.1rem; margin: 0; line-height: 1;">${mod.icon}</div>
                        <div class="pres-card-title" style="font-size: 1.2rem; font-weight: 800; color: var(--text-primary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${mod.name}</div>
                    </div>
                </div>
                <div style="display: flex; flex-direction: column; gap: 0.6rem; flex: 1; overflow-y: auto; min-height: 0; padding-right: 0.5rem;">
                    ${commit.goals.map((goal, idx) => {
                        ...
                        return `
                            <div style="display: flex; align-items: flex-start; gap: 0.6rem; font-size: 0.85rem; color: var(--text-secondary); line-height: 1.4; background: rgba(0,0,0,0.02); padding: 0.6rem; border-radius: 6px; border: 1px solid rgba(0,0,0,0.03);">
