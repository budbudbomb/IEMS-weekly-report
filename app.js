...
const MODULES = {
    ...
    samadhan: {
        ...
        finalStatus: 'In progress',
        businessDevelopment: 'Malviya sir reviewed it once, gave some changes which were implemented. But he doesn\'t give any formal approval. Pradeep sir has approved it. He asked to show it to Sanju ma\'am but Sanju ma\'am said that she has no knowledge of this domain',
        dependency: '',
        clientReviewPoints: [],
        documentation: { processFlow: 'Done', userManual: 'Done', conceptNote: '-' },
        team: ['Ashmit'],
    ...
    expenditure: {
        ...
        finalStatus: 'In Progress',
        businessDevelopment: 'All approved from Pradeep sir. He asked to show it to Sanju ma\'am (given that she\'s the IT head) or Shrivastava sir',
        dependency: '',
        clientReviewPoints: [],
    ...
    sugamta: {
        ...
        icon: 'सु',
        color: '#f43f5e',
        requirementGathering: 'Got the requirement for everything related to the commutation of "polling parties" in "Panchayat elections"\n\nPending For :\n- Nagariya Elections\n- Sector Officer\n- Sector Magistrate',
        staticScreens: { creation: 'In Progress', presentation: 'Done - Client has asked to add (Vehicle fitness and Insurance check)', status: 'Done' },
        finalStatus: 'In progress',
        businessDevelopment: 'Sanju ma\'am is mainly interested in this module. She won\'t review Preksha alone. Aiming to present Election Observer in the coming week while conveying the progress in Pithasin module.\nNeed to visit Shakya sir to get his review on the Static screen (as he gave a few suggestions regarding the module)',
    ...
    evm: {
        ...
        finalStatus: 'In progress',
        businessDevelopment: 'Got the requirement from Mukul Gupta (DySecretary). The app needs to go live by Monday',
        dependency: '',
        clientReviewPoints: [],
...
function countClientReviewPoints(mod) {
    ...
    mod.userTypes.forEach(ut => {
        ut.categories.forEach(cat => {
            cat.pages.forEach(p => {
                if (p.clientReview && p.clientReview.review && p.clientReview.review !== '-' && p.clientReview.review !== '') count++;
            });
        });
    });
    return count;
...
function renderDetailHeader() {
    ...
    document.getElementById('detail-header').innerHTML = `
        <div class="detail-module-icon" style="background: ${mod.color}15; color: ${mod.color}; border: 1px solid ${mod.color}30">${mod.icon}</div>
...
function renderOverview(container) {
    ...
    if (mod.businessDevelopment) {
        html += `
            <div class="alert-card alert-info" style="margin-bottom:1rem">
                ...
                <div class="alert-body">${mod.businessDevelopment.replace(/\n/g, '<br>')}</div>
            </div>
        `;
    }
    ...
    // Per User-Type Static Screens (if they differ)
    const hasUserTypeInfo = mod.userTypes.some(ut => ut.staticScreens || ut.reqGathering);
    if (hasUserTypeInfo) {
        html += `
            <div class="section-header" style="margin-top:1rem">
                <div class="section-icon">📊</div>
                <div>
                    <div class="section-title">Per User-Type Overview</div>
                    <div class="section-subtitle">Requirement gathering and static screen status by user type</div>
                </div>
            </div>
            <div class="table-section">
                <table class="data-table">
                    <thead>
                        <tr>
                            <th>User Type</th>
                            <th class="text-center col-status">Req. Gathering</th>
                            <th class="text-center col-status">Static Creation</th>
                            <th class="text-center col-status">Static Presentation</th>
                            <th class="text-center col-status">Static Status</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${mod.userTypes.filter(ut => ut.staticScreens || ut.reqGathering).map(ut => `
                            <tr>
                                <td>${ut.name}</td>
                                <td class="text-center col-status">${getStatusBadge(ut.reqGathering || mod.requirementGathering)}</td>
                                <td class="text-center col-status">${getStatusBadge(ut.staticScreens ? ut.staticScreens.creation : mod.staticScreens.creation)}</td>
                                <td class="text-center col-status">${getStatusBadge(ut.staticScreens ? ut.staticScreens.presentation : mod.staticScreens.presentation)}</td>
                                <td class="text-center col-status">${getStatusBadge(ut.staticScreens ? ut.staticScreens.status : mod.staticScreens.status)}</td>
                            </tr>
                        `).join('')}
                    </tbody>
                </table>
